...
// POST /api/v1/officer/points/remove — take points from an assigned player
// back to the officer.
export async function POST(request: Request) {
  const auth = await requireApiRole('officer');
  if ('error' in auth) return auth.error;

  const limited = enforceRateLimit(`remove:${auth.profile.id}`, 30);
  ...
  const { data, error } = await supabase.rpc('officer_remove_points', {
